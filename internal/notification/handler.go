...
// NewHandler wires the notification CRUD routes behind the provided auth middleware.
...
func NewHandler(svc *Service, authMW func(http.Handler) http.Handler) http.Handler {
	...
	// Dedicated sub-route for marking as read (idempotent).
	mux.HandleFunc("PUT /notifications/{id}/read", h.markAsRead)
...
// The operation is idempotent: always returns 200 with the current state.
func (h *handler) markAsRead(w http.ResponseWriter, r *http.Request) {
	id := r.PathValue("id")
	n, err := h.svc.MarkAsRead(r.Context(), id)
	if err != nil {
		if errors.Is(err, ErrNotFound) {
			writeError(w, http.StatusNotFound, "notification not found")
			return
